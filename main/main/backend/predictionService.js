...
class PredictionService {
    constructor() {
        this.cache = new node_cache_1.default({ stdTTL: CACHE_TTL_SECONDS, checkperiod: 120 });
    }
    runPrediction(input) {
        ...
        (0, storage_1.persistResult)(result);
        return result;
    }
    getHistory() {
        return (0, storage_1.loadHistory)();
    }
    clearCache() {
        this.cache.flushAll();
    }
    computeSaraProjection(input) {
        const { saraProfile, context } = input;
        const totalSara = saraProfile.saturates +
        ...
        const resolveCongealMultiplier = (pourPointF, onSiteF) => {
        ...
        const effectivePourPointF = context.pourPointF !== null
            ? context.pourPointF
        ...
        const stabilityIndex = Number(Math.max(0, Math.min(100, Number.isFinite(stabilityBase) ? stabilityBase : 0)).toFixed(2));
        const crystallizationTime = Number(Math.max(1.5, Math.min(36, 6.5 +
            viscosityFactor * 4.8 +
            waxFraction * 10 +
            congealRatioForStability * 6 -
            normalized.aromatics * 2.3 +
            normalized.resins * 1.4)).toFixed(2));
        const pourPoint = Number(Math.max(-45, Math.min(35, pourPointC +
            waxFraction * 22 -
            normalized.aromatics * 6 +
            normalized.asphaltenes * 5)).toFixed(2));
        const recommendations = [];
        if (congealRatio >= 1) {
            recommendations.push('Congeal ratio exceeds 1.0. Reduce saturates or asphaltenes and increase aromatics/resins immediately.');
        }
        else if (congealRatio >= 0.6) {
            recommendations.push('Congeal ratio is trending upward. Increase aromatic or resin content to improve dispersion stability.');
        }
        if (stabilityIndex < 45) {
            recommendations.push('Stability is low. Adjust blend toward higher aromatics/resins and reduce wax deposition risk.');
        }
        ...
        if (crystallizationTime > 18) {
            recommendations.push('Crystallization is slow. Evaluate viscosity reducers or increase agitation in the system.');
        }
        else if (crystallizationTime < 6) {
            recommendations.push('Crystallization is fast. Lower wax deposition or increase resin content to moderate crystal formation.');
        }
        if (pourPoint > 15) {
