...
exports.calculateScalePrediction = calculateScalePrediction;
/**
 * Molar masses (g/mol)
 */
const MOLAR_MASS = {
 ...
 */
function mgLToMolality(mgL, molarMass, waterDensity = 1.0) {
    if (mgL <= 0 || molarMass <= 0)
        return 0;
 ...
 */
function calculateKspBarite(temperatureF, pressurePsia) {
 ...
 * R = 83.144 (from VBA)
 */
function calculateKspCalcite(temperatureF, pressurePsia) {
    const TC = (temperatureF - 32) * (5 / 9);
    const TK = TC + 273.15;
 ...
 */
function calculateKspFeS(temperatureF, pressurePsia) {
    const TC = (temperatureF - 32) * (5 / 9);
    const TK = TC + 273.15;
 ...
 */
function calculateKspZnS(temperatureF, pressurePsia) {
    const TC = (temperatureF - 32) * (5 / 9);
    const TK = TC + 273.15;
    const Patm = pressurePsia / 14.7;
 ...
 */
function calculateKspCaF2(temperatureF, pressurePsia) {
    const TC = (temperatureF - 32) * (5 / 9);
    const TK = TC + 273.15;
    const Patm = pressurePsia / 14.7;
 ...
 * R = 83.144 (from VBA)
 */
function calculateKspFeCO3(temperatureF, pressurePsia) {
    const TC = (temperatureF - 32) * (5 / 9);
    const TK = TC + 273.15;
    const Patm = pressurePsia / 14.7;
    const R = 83.144; // VBA constant
 ...
 */
function calculateKspZnCO3(temperatureF, pressurePsia) {
    const TC = (temperatureF - 32) * (5 / 9);
    const TK = TC + 273.15;
    const Patm = pressurePsia / 14.7;
 ...
 */
function calculateKspPbS(temperatureF, pressurePsia) {
    const TC = (temperatureF - 32) * (5 / 9);
    const TK = TC + 273.15;
    const Patm = pressurePsia / 14.7;
 ...
 */
function calculateActivityCoefficient(charge, ionicStrength, temperatureC) {
    if (ionicStrength <= 0)
        return 1.0;
 ...
 */
function calculateIonicStrength(brine, waterDensity = 1.0) {
    const molalities = {
 ...
 */
function calculateBariteSI(brine, temperatureF, pressurePsia, inhibitors) {
    const TC = (temperatureF - 32) * (5 / 9);
 ...
 */
function calculatepHForSulfides(gasAcidity, temperatureF, pressurePsia, brine, waterDensity) {
    const TC = (temperatureF - 32) * (5 / 9);
    const TK = TC + 273.15;
    const Patm = pressurePsia / 14.7;
    const R = 83.144;
 ...
 */
function calculateHSFromH2S(totalH2Saq, pH, temperatureF, pressurePsia, ionicStrength, waterDensity) {
    if (totalH2Saq <= 0)
        return 0;
    const TC = (temperatureF - 32) * (5 / 9);
    const TK = TC + 273.15;
 ...
 */
function calculateSulfidesSIDetailed(brine, gasAcidity, temperatureF, pressurePsia, inhibitors) {
    const TC = (temperatureF - 32) * (5 / 9);
    const TK = TC + 273.15;
    const waterDensity = brine.calcDensity ?? 1.0;
    const ionicStrength = calculateIonicStrength(brine, waterDensity);
 ...
 */
function calculateSulfidesSI(brine, gasAcidity, temperatureF, pressurePsia, inhibitors) {
 ...
 */
function calculateCalciteSI(brine, gasAcidity, temperatureF, pressurePsia, inhibitors) {
    const TC = (temperatureF - 32) * (5 / 9);
    const TK = TC + 273.15;
    const waterDensity = brine.calcDensity ?? 1.0;
    // Convert to molality
 ...
 */
function calculatePrecipitationAmount(mineralType, SI, brine, temperatureF, pressurePsia, gasAcidity) {
    if (SI <= 0)
        return 0;
    const TC = (temperatureF - 32) * (5 / 9);
    const waterDensity = brine.calcDensity ?? 1.0;
 ...
 */
function getScalingStatus(SI) {
    if (SI > 1.0)
        return 'Severe Scaling';
    if (SI > 0.5)
        return 'Moderate Scaling';
    if (SI > 0)
        return 'Light Scaling';
    return 'No Scaling';
}
/**
 * Main calculation function
 */
function calculateScalePrediction(input) {
    ...
    let mineralDetails;
    if (mineralType === 'barite') {
        siBH = calculateBariteSI(brineChemistry, operatingConditions.initialTemperature, operatingConditions.initialPressure, inhibitors);
        siWH = calculateBariteSI(brineChemistry, operatingConditions.finalTemperature, operatingConditions.finalPressure, inhibitors);
        ...
        mineralDetails = [{
                ...
                siWH,
                deltaSI: siWH - siBH,
                amountToPrecipitate: calculatePrecipitationAmount(mineralType, siWH, brineChemistry, operatingConditions.finalTemperature, operatingConditions.finalPressure, gasAcidity)
            }];
    }
    else if (mineralType === 'calcite') {
        siBH = calculateCalciteSI(brineChemistry, gasAcidity, operatingConditions.initialTemperature, operatingConditions.initialPressure, inhibitors);
        siWH = calculateCalciteSI(brineChemistry, gasAcidity, operatingConditions.finalTemperature, operatingConditions.finalPressure, inhibitors);
        ...
        const bhResult = calculateSulfidesSIDetailed(brineChemistry, gasAcidity, operatingConditions.initialTemperature, operatingConditions.initialPressure, inhibitors);
        const whResult = calculateSulfidesSIDetailed(brineChemistry, gasAcidity, operatingConditions.finalTemperature, operatingConditions.finalPressure, inhibitors);
        siBH = bhResult.maxSI;
        siWH = whResult.maxSI;
    ...
    const amountToPrecipitate = calculatePrecipitationAmount(mineralType, siWH, brineChemistry, operatingConditions.finalTemperature, operatingConditions.finalPressure, gasAcidity);
    return {
        mineralType,
