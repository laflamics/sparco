export type ScaleMethod = 'stiffDavis' | 'oddoTomson';
...
export class ScaleService {
  calculate(input: ScaleInput): ScaleResult {
  ...
  private calculateEmpirical(input: ScaleInput): ScaleResult {
    ...
    const amountScale =
      Math.round(PTB_MULTIPLIER_SIMPLE * saturationIndex * 10000) / 10000;

    let status: ScaleResult['status'];
  ...
  private calculateOddoTomson(input: ScaleInput): ScaleResult {
    ...
    const gammaCo3 = this.activityCoefficient(
      2,
      ION_SIZE.carbonate,
      ionicStrength,
      temperatureC
    ...
    const amountScale = parseFloat(
      (PTB_MULTIPLIER_ODDO * saturationIndexRaw).toFixed(7)
    );
  ...
  private mgPerLToMolPerL(value: number, molarMass: number): number {
    return value <= 0 ? 0 : value / (molarMass * 1000);
  }

  private calculateIonicStrength(concentrations: Record<keyof typeof VALENCE, number>) {
    let sum = 0;
    (Object.keys(concentrations) as Array<keyof typeof VALENCE>).forEach((key) => {
  ...
  private deriveCarbonate(
    bicarbonateMol: number,
    ph: number,
    temperatureC: number
  ): number {
    const temperatureK = temperatureC + 273.15;
  ...
  private calciteKsp(temperatureC: number): number {
    const temperatureK = temperatureC + 273.15;
    const log10Ksp =
      -171.9065 -
